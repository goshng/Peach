...
sub setOnsites {
	...
	my $num_cols; my $parentOns_ref;  my $oldNewSiteMap_ref; my $duplicateColsHash_ref; my $matrix_ref; my $rootSeqID;
	($num_cols, $parentOns_ref, $oldNewSiteMap_ref, $duplicateColsHash_ref, $matrix_ref, $rootSeqID) = @_;

	if ( $self->site() >= 0 ) {
		# map new site (of unique columns matrix) to old site (of duplicate columns matrix)
		# and add the duplicates of the old site

		my $site = $self->site(); # get new site
		$self->site($oldNewSiteMap_ref->[$site]); # set to corresponding old site

		my @array;

		# reset all sites to their corresponding old sites
		my $allsites_ref = $self->allsites();
		for (my $i = 0; $i < scalar(@{$allsites_ref}); $i++) {
			$allsites_ref->[$i] = $oldNewSiteMap_ref->[$allsites_ref->[$i]];
			push @array, $allsites_ref->[$i]; # old site must be propagated
			if ( defined $duplicateColsHash_ref->{$allsites_ref->[$i]} ) { # if old site has duplicates
				push @array, @{$duplicateColsHash_ref->{$allsites_ref->[$i]}}; # propagate duplicates of old site also
			}
		}

		$self->allsites(\@array); # site + duplicates = all sites

		my @onsites = @{[@{$parentOns_ref}, @array]};
...
sub findOnSites {
	...
	if ( $self->setOnsites($num_cols, $parentOns_ref, $oldNewSiteMap_ref, $duplicateColsHash_ref, $matrix_ref, $rootSeqID) == -1) {
		return -1;
	}
...
sub pointToParent {
	...
	if (defined $parent) {
		...
		croak "Cannot go against directed edge. Wrong root chosen for T_bar.\n" if ($self->seekParent() == 0);
...
sub new {
	...
	my $class = ref($type) || $type;
        ...
        my $root = Tree::Node->new(-1); #root node does not represent any site mutation
...
sub build {
	...
	for (my $i = 0; $i < $size; $i++) {
		if ($array_ref->[$i] == $site) {
			my $child = Tree::Node->new($i); # zero-based site index
