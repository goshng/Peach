...
parse_locus_info (int li, int *uinext, char *cc, int *fpstri, char fpstr[])
{
  ...
  if (L[li].numlines <= 1) // JH added this to trap an input file error  1/14/2011
  {
    IM_err (IMERR_INPUTFILEINVALID,"Each locus must have at least two gene copies. Locus %d (%s) has %d.",li,L[li].name, L[li].numlines);
  }
  for (i = 0; i < L[li].numgenes; i++)
  {
    L[li].pairs[i] = -1;
